...
function Sidebar({ report, selId, loaded, isMobile, sidebarOpen, search, width, onWidthChange, onClose, onSearch, onOverview, onSelectPr, onLoadMore, onRefresh, refreshing, refreshStatus }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      {/* Scrollable: just the review list */}
...
function SidebarLoadMore({ onClick }: { onClick: () => void }) {
  ...
  return (
    <div
      ...
      onMouseLeave={() => setHovered(false)}
      style={{
        padding: '9px', textAlign: 'center', color: 'var(--color-accent)',
        fontSize: '11.5px', fontWeight: 500, cursor: 'pointer',
        borderRadius: '6px', marginTop: '4px', userSelect: 'none',
        background: hovered ? 'var(--color-sidebar-raised)' : 'transparent', transition: 'background 0.1s',
      }}
    >
      Load more reviews
    </div>
...
export default function Dashboard({ data, reportId, onRefresh, refreshing, refreshStatus }: Props) {
  ...
  const [sidebarWidth, setSidebarWidth] = useSidebarWidth('pr-review')
  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: 'var(--color-background)' }}>
      ...
      {showJson && (
        <RawJsonModal
          data={selectedPr ?? report}
          title={selectedPr ? `PR #${selectedPr.prNumber}` : (report.title ?? 'PR Review Report')}
          subtitle={selectedPr ? 'Raw JSON — selected review' : 'Raw JSON — full report'}
