...
test.describe('PR Review dashboard', () => {
  ...
  test('schema v4 displays rounded AI-credit usage with estimated USD', async ({ page }) => {
    ...
    await expect(page.getByText('16 AIC · ≈$0.16', { exact: true })).toBeVisible()
    ...
    await expect(page.getByRole('main').getByText('1 AIC · ≈$0.01', { exact: true })).toBeVisible()
  })
  ...
  test('schema v7 distinguishes official and attributed AIC and shows grounding health', async ({ page }) => {
    ...
    await expect(page.getByText('Unsupported schema version')).not.toBeVisible()
    await expect(page.getByText('Organization Copilot Usage', { exact: true })).toBeVisible()
    await expect(page.getByText('19 AIC · ≈$0.19', { exact: true })).toBeVisible()
    await expect(page.getByText('Review-agent AIC', { exact: true })).toBeVisible()
    const reviewAic = page.getByRole('group', { name: 'Review-agent AIC' })
    await expect(reviewAic.getByText('1 AIC', { exact: true })).toBeVisible()
    await expect(reviewAic.getByText('≈$0.01', { exact: true })).toBeVisible()
    await expect(page.getByText('1 of 1 PRs · 100% coverage', { exact: true })).toBeVisible()
    const groundingHealth = page.getByRole('group', { name: 'Grounding Health' })
    await expect(groundingHealth.getByText('0%', { exact: true })).toBeVisible()
    await expect(groundingHealth.getByText('0 healthy · 1 degraded', { exact: true })).toBeVisible()
  ...
  test('schema v8 shows provider token coverage and detailed usage anatomy', async ({ page }) => {
    await page.goto('/?report=pr-review&id=v8&_fixture=dev')
    await expect(page.getByText('Unsupported schema version')).not.toBeVisible()
    await expect(page.locator('aside').getByText('7 AIC', { exact: true })).toBeVisible()
    await expect(page.getByText('Organization Copilot Usage', { exact: true })).not.toBeVisible()
    const reviewAic = page.getByRole('group', { name: 'Review-agent AIC' })
    await expect(reviewAic.getByText('7 AIC', { exact: true })).toBeVisible()
    await expect(reviewAic.getByText('≈$0.07', { exact: true })).toBeVisible()
    ...
    const tokenTrafficCard = page.getByText('Token Traffic per Review').locator('../..')
    const tokenChart = tokenTrafficCard.getByTestId('token-review-hover-layer')
    const chartBox = await tokenChart.boundingBox()
    expect(chartBox).not.toBeNull()
    await tokenChart.hover({ position: { x: chartBox!.width / 2, y: chartBox!.height / 2 } })
    const tokenTooltip = tokenTrafficCard.getByRole('tooltip')
    await expect(tokenTooltip).toContainText('26.4k total tokens - 7 AIC ≈$0.07')
    await expect(tokenTooltip).not.toContainText('token cost unavailable')
    await tokenChart.focus()
    await page.keyboard.press('ArrowRight')
    await expect(tokenTooltip).toContainText('#18660')
